...
const PRIORITY_COLORS = {
  ...
  medium:   { bg: '#fff3e0', text: '#c25a00', border: '#fed7aa' },
  high:     { bg: '#fce8e8', text: '#c62828', border: '#fca5a5' },
...
function RaiseTicketModal({ onClose, onCreated }) {
  ...
  const [form, setForm] = useState({
    ...
    in_warranty_period: false,
    location_lat:       '',
    location_lng:       '',
  })

  const [sop, setSop] = useState({
    panel_board_checked:    false,
    three_phase_verified:   false,
    fuse_mcb_checked:       false,
    video_recorded:         false,
    photos_taken:           false,
    person_will_be_present: false,
  })
  ...
  useEffect(() => {
    if (!form.farm_id) { setDevices([]); setForm(f => ({ ...f, uid: '' })); return }
    api.get(`/farms/${form.farm_id}/devices`)
  ...
  useEffect(() => {
    ...
    if (phoneOpen) document.addEventListener('click', close)
  ...
  // Replace handleCapture with this:
const handleCapture = async (file, type) => {
  ...
  try {
    ...
    toast.error(detail || `Upload failed — ${err.message}`)
  ...
  const handleSubmit = async () => {
    ...
    try {
      const res = await api.post('/services', {
        ...
        sop_checklist:      sop,
        photo_evidence:     [capturedPhoto.url, capturedVideo?.url].filter(Boolean),
        contact_person:     form.contact_person,
        contact_number:     selectedCountry.code + phoneNumber,
        in_warranty_period: form.in_warranty_period,
        ...
        <div className="flex-1 overflow-y-auto p-5 bg-white">
          ...
          {step === 1 && (
            <div className="flex flex-col gap-4">
              ...
              <Field label="Device UID" required hint="Select farm first to filter devices, or type UID manually">
                {devices.length > 0 ? (
                  <Select value={form.uid} onChange={e => set('uid', e.target.value)}>
                    <option value="">— Select device —</option>
                    {devices.map(d => (
                      <option key={d.uid} value={d.uid}>{d.borewell_name || d.uid} — {d.uid}</option>
                    ))}
                  </Select>
                ) : (
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <Field label="Contact Number" required>
                  <div className="flex rounded-xl border-2 border-gray-200 focus-within:border-[#106f30] transition-all bg-white">
                    <div style={{ position: 'relative', flexShrink: 0 }} onClick={e => e.stopPropagation()}>
                      <button type="button" onClick={() => setPhoneOpen(o => !o)}
                        className="flex items-center gap-1.5 px-3 py-2.5 bg-gray-50 hover:bg-gray-100 border-r border-gray-200 text-sm h-full rounded-l-xl whitespace-nowrap">
                        <span style={{ fontSize: '16px' }}>{selectedCountry.flag}</span>
                        <span className="text-gray-700 font-medium text-xs">{selectedCountry.code}</span>
                      ...
                      {phoneOpen && (
                        <div style={{
                          ...
                          marginTop: '4px', width: '220px', maxHeight: '240px', overflowY: 'auto'
                        }}>
                          {COUNTRIES.map(c => (
                    ...
                    <input type="tel" placeholder="98765 43210"
                      ...
                      className="flex-1 px-3 py-2.5 text-sm bg-white focus:outline-none rounded-r-xl min-w-0" />
                  </div>
                </Field>
              </div>
              ...
              <label className="flex items-center gap-3 p-3 bg-gray-50 rounded-xl cursor-pointer">
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-400">Check if device is still under warranty</p>
            ...
            <div className="flex flex-col gap-4">
              <div className="bg-amber-50 border border-amber-200 rounded-xl p-4">
                ...
                <p className="text-xs text-amber-700">
                  As per KrishiHrudya SOP, all the following steps must be completed
                  before a service ticket can be raised.
                </p>
              </div>
              ...
              {!allSopChecked ? (
                <div className="bg-red-50 border border-red-200 rounded-xl p-3">
                  <p className="text-xs text-red-700 font-medium">
                    ❌ {Object.values(sop).filter(Boolean).length} of 6 items completed. All 6 must be checked to proceed.
                  </p>
                </div>
              ) : (
                <div className="bg-green-50 border border-green-200 rounded-xl p-3">
                  <p className="text-xs text-green-700 font-medium">✅ All SOP items completed. You may proceed.</p>
                </div>
              )}
            </div>
          )}
          ...
          {step === 3 && (
            <div className="flex flex-col gap-5">
                ...
                {!capturedPhoto ? (
                  <label className={`flex flex-col items-center justify-center gap-3 p-8 border-2 border-dashed rounded-xl cursor-pointer transition-all
                    ...
                    <div className="text-center">
                      ...
                      <p className="text-xs text-gray-400 mt-1">Take a live photo of the issue</p>
                    </div>
                    {!uploading && (
                      <span className="px-4 py-2 rounded-xl text-white text-sm font-medium"
                  ...
                  <div className="relative rounded-xl overflow-hidden border-2 border-green-300">
                    <img src={capturedPhoto.localUrl} alt="Captured"
                      className="w-full max-h-56 object-cover" />
                    <div className="absolute top-2 right-2 flex gap-2">
                      <label className="px-3 py-1.5 bg-white/90 backdrop-blur rounded-lg text-xs font-medium text-gray-700 cursor-pointer hover:bg-white border border-gray-200">
                        🔄 Retake
                        <input type="file" accept="image/*" capture="environment" className="hidden"
                          onChange={e => { if (e.target.files[0]) handleCapture(e.target.files[0], 'photo') }} />
                      </label>
                      <button type="button" onClick={() => setCapturedPhoto(null)}
                        className="px-3 py-1.5 bg-red-500 rounded-lg text-xs font-medium text-white hover:bg-red-600">
                        ✕ Remove
                      </button>
                    </div>
              ...
              <div className="flex flex-col gap-3">
                <p className="text-sm font-medium text-gray-700">
                  🎬 Video
                  <span className="text-xs text-gray-400 font-normal ml-2">1 video up to 1 minute (optional)</span>
                </p>
                {!capturedVideo ? (
                  <label className={`flex flex-col items-center justify-center gap-3 p-6 border-2 border-dashed rounded-xl cursor-pointer transition-all
                    ...
                    <div className="text-center">
                      <p className="text-sm font-semibold text-gray-700">
                        {uploading ? '⏳ Uploading...' : 'Tap to record video'}
                      </p>
                      <p className="text-xs text-gray-400 mt-1">Keep it under 1 minute</p>
                    </div>
                    {!uploading && (
                  ...
                  <div className="rounded-xl overflow-hidden border-2 border-green-300 bg-black">
                    ...
                    <div className="bg-white px-3 py-2 flex items-center justify-between">
                      ...
                      <div className="flex gap-2">
                        <label className="px-3 py-1.5 bg-gray-100 rounded-lg text-xs font-medium text-gray-700 cursor-pointer hover:bg-gray-200">
                          🔄 Re-record
                          <input type="file" accept="video/*" capture="environment" className="hidden"
                            onChange={e => { if (e.target.files[0]) handleCapture(e.target.files[0], 'video') }} />
                        </label>
              ...
              {uploading && (
                <div className="flex items-center gap-3 p-3 bg-blue-50 border border-blue-200 rounded-xl">
                  ...
                  <p className="text-xs text-blue-700 font-medium">Uploading to server, please wait...</p>
                </div>
              )}
...
function TicketDetail({ ticketId, onClose, onUpdated, isAdmin }) {
  ...
  const [statusForm, setStatusForm] = useState({
    status: '', service_mode: null, assigned_dealer_id: '', resolution_notes: '',
    device_received_at: '', device_returned_at: '',
  ...
  const fetchTicket = async () => {
    setLoading(true)
    try {
      const res = await api.get(`/services/${ticketId}`)
      setTicket(res.data)
      setStatusForm(f => ({ ...f, status: res.data.status }))
      if (res.data.location_lat && res.data.location_lng) {
        api.get(`/services/nearby-dealers?lat=${res.data.location_lat}&lng=${res.data.location_lng}`)
  ...
  const handleUpdate = async () => {
    ...
    setSaving(true)
    try {
      await api.put(`/services/${ticketId}`, statusForm)
  ...
  const mediaBase = 'http://187.127.139.240'
    ...
    <div className="fixed inset-0 bg-black/40 flex items-end lg:items-center justify-end z-50">
      <div className="bg-white w-full lg:w-[520px] h-full lg:h-full overflow-y-auto shadow-2xl">
        ...
        <div className="p-5 flex flex-col gap-5">
          ...
          <div className="bg-gray-50 rounded-xl p-4">
            <p className="text-xs font-semibold text-gray-400 uppercase tracking-wide mb-3">Device Information</p>
            <div className="grid grid-cols-2 gap-3">
              {[
                ['Device UID',  ticket.uid || '—'],
                ['Farm',        ticket.farm_name || '—'],
                ['Customer',    ticket.customer_name || '—'],
                ['Customer ID', ticket.customer_code || '—'],
          ...
          {/* Evidence — show images and videos inline */}
            ...
            <div>
              ...
              <div className="flex flex-col gap-3">
                {ticket.photo_evidence.map((url, i) => (
                  <div key={i}>
                    {isVideo(url) ? (
                      <video src={mediaBase + url} controls
                        className="w-full rounded-xl border border-gray-200 max-h-52" />
                    ) : isImage(url) ? (
                      <a href={mediaBase + url} target="_blank" rel="noreferrer">
                        <img src={mediaBase + url} alt={`Evidence ${i + 1}`}
                          className="w-full rounded-xl border border-gray-200 max-h-52 object-cover hover:opacity-90 transition-all" />
                      </a>
                    ) : (
                      <a href={url} target="_blank" rel="noreferrer"
                        className="flex items-center gap-2 px-3 py-2.5 bg-blue-50 border border-blue-200 rounded-xl text-xs text-blue-700 hover:bg-blue-100">
                        <span>🔗</span><span className="truncate">{url}</span><span className="ml-auto">↗</span>
                      </a>
                    )}
                  </div>
                ))}
          ...
          {isOfficeMode && (
            <div className="bg-amber-50 border border-amber-200 rounded-xl p-4">
              <p className="text-xs font-semibold text-amber-700 uppercase tracking-wide mb-3">Device Tracking</p>
              <div className="flex flex-col gap-2">
                {[
                  ['Device Received',  formatDate(ticket.device_received_at)],
                  ['Condition on In',  ticket.device_condition_in || '—'],
                  ['Device Returned',  formatDate(ticket.device_returned_at)],
                  ['Condition on Out', ticket.device_condition_out || '—'],
                ].map(([label, value]) => (
                  <div key={label} className="flex justify-between gap-4">
                    <span className="text-xs text-amber-600">{label}</span>
                    <span className="text-xs font-medium text-amber-800">{value}</span>
                  </div>
                ))}
              </div>
            </div>
              ...
              <div className="flex flex-col gap-4">
                ...
                {(!ticket.service_mode || ticket.status === 'open') && (
                  <Field label="Service Mode" required
                    hint="Decide whether technician visits site or customer brings device to office">
                    <div className="flex gap-2">
                ...
                {!isOfficeMode && (
                  <Field label="Assign Dealer"
                    hint={dealers.length > 0 ? `${dealers.length} dealer(s) found nearby` : 'No nearby dealers found'}>
                    <Select value={statusForm.assigned_dealer_id}
                      ...
                      {dealers.map(d => (
                        <option key={d.dealer_id} value={d.dealer_id}>
                          {d.full_name} ({d.dealer_code}){d.distance_km ? ` — ${d.distance_km} km away` : ''}
                        </option>
                      ))}
                    </Select>
                  </Field>
                ...
                {isOfficeMode && ['device_received', 'repaired', 'closed'].includes(statusForm.status) && (
                  <div className="bg-amber-50 rounded-xl p-4 flex flex-col gap-3">
                    <p className="text-xs font-semibold text-amber-700">Device Tracking</p>
                    <Field label="Device Received At">
                      <Input type="datetime-local" value={statusForm.device_received_at}
                        onChange={e => setStatusForm(f => ({ ...f, device_received_at: e.target.value }))} />
                    </Field>
                    <Field label="Device Condition (On Receive)">
                      <Input placeholder="e.g. Screen cracked, power issue..." value={statusForm.device_condition_in}
                        onChange={e => setStatusForm(f => ({ ...f, device_condition_in: e.target.value }))} />
                    </Field>
                    {statusForm.status === 'closed' && (
                      <>
                        <Field label="Device Returned At">
                          <Input type="datetime-local" value={statusForm.device_returned_at}
                            onChange={e => setStatusForm(f => ({ ...f, device_returned_at: e.target.value }))} />
                        </Field>
                        <Field label="Device Condition (On Return)">
                          <Input placeholder="e.g. Repaired, tested OK..." value={statusForm.device_condition_out}
                            onChange={e => setStatusForm(f => ({ ...f, device_condition_out: e.target.value }))} />
                        </Field>
                      </>
                    )}
                  </div>
                ...
                <Field label="Resolution Notes" hint="What was the issue? How was it resolved?">
                ...
                <Field label="Internal Note" hint="Added to timeline">
    ...
    <div className="max-w-6xl">
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-0.5">{total} ticket{total !== 1 ? 's' : ''} total</p>
        </div>
        {can('services_view') && (
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-2xl p-12 text-center border border-gray-100 shadow-sm">
          ...
          <p className="text-gray-400 text-xs mt-1">
            {filterStatus || filterCategory ? 'Try clearing the filters' : 'Raise a ticket to get started'}
          </p>
        </div>
      ) : (
        <div className="flex flex-col gap-3">
                ...
                <div className="flex items-start justify-between gap-4">
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-3 mt-2 flex-wrap">
                      ...
                      {ticket.farm_name && <span className="text-xs text-gray-400">🌾 {ticket.farm_name}</span>}
